...
<div class="login">
    ...
    <div class="reg_tab login-inp" data-tab="email">
        <a class="email active" >邮箱验证</a>
        <a class="phone " >手机号验证</a>
    </div>


    <div class="login-form">
        <form class="submit-ajax" action="<?php echo U('forget_pwd');?>">

         <!-- // TODO 修改注册-->
          <div class="reg_email">
            <div class="login-inp" style="background: #f4f4f4;border: none;">
                <label style="color: #999;">注册账号</label>
                <input type="text" name="username_email" id="username_email" placeholder="请输入邮箱" style="color: #999;">
            </div>

            <div class="login-inp" style="background: #f4f4f4;border: none;">
                <label style="color: #999;">验证码</label>
                <input type="text" name="code_email" id="code_email" placeholder="请输入邮箱验证码" style="color: #999;width:40%">
                <input type="button" id="getcode_email" value="发送验证码" style="background:#f92e4a; color:#FFF; border-radius:5px; height:30px; line-height:30px;" />
            </div>
          </div>
          <div class="reg_phone" style="display: none;">
            ...
            <div class="login-inp" style="background: #f4f4f4;border: none;">
                <label style="color: #999;">验证码</label>
                <input type="text" name="code" id="code" placeholder="请输入手机验证码" style="color: #999;width:40%">
                <input type="button" id="getcode" value="发送验证码" style="background:#f92e4a; color:#FFF; border-radius:5px; height:30px; line-height:30px;" />
            </div>
          </div>
       <!-- // TODO 修改注册-->
        ...
        <div class="login-inp" style="border: none;background: #f92e4a;">
   ...
   <a href="<?php echo U('login');?>">
        	<p style="width: 150px;height: 60px;line-height: 60px;margin: 20px auto 0; color: #f92e4a; "> <span style="width: 20px;height: 20px;border: 1px #f92e4a solid;display: inline-block;margin: 20px 6px 20px 40px;text-align: center;float: left;line-height: 20px;border-radius: 20px;text-align: center;color: #f92e4a;">→</span> 去登陆</p>
        </a>
  	...
  	$(document).ready(function(){
    	$("#getcode").click(function(){
        	var username = $.trim( $("#username").val() );
          	if( username == ''  )
            {
                alert_fang( '请输入手机号码' ); return false;
            }
          	if( username.length != 11 )
            {
                alert_fang( '手机号码不正确' ); return false;
            }
    	...
    	$("#getcode_email").click(function(){
        	var username = $.trim( $("#username_email").val() );
          	if( username == ''  )
            {
                alert_fang( '请输入邮箱' ); return false;
            }
            ...
            if (!isok) {
          	...
          	$.get( "<?php echo U('getcode_email');?>",{tel:username,forget:1},function( res ){
                alert_fang( res.msg );
            		if( res.code == 1 )
                    {
                    	daojishi( 60,2 );
